...
local activeCooldowns = {} -- [slotIndex] = {endTime = t, duration = d}
...
AbilityUsedRemote.OnClientEvent:Connect(function(slotIndex, abilityId)
    if slotIndex == "Counter" then
        local data = AbilityData.GetAbility("Counter")
        if data and data.Cooldown then
            -- Counter has its own UI if you want later
        end
        return
    end
